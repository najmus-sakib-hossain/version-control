...
async fn test_content_addressable_storage() -> Result<()> {
    ...
    println!("   ✓ Deduplication verified: {} == {}", 
        &blob1.metadata.hash[..8], &blob2.metadata.hash[..8]);
    
    // Create blob with different content
    let content3 = b"Different content".to_vec();
    let blob3 = Blob::from_content("test3.txt", content3);
    
    assert_ne!(blob1.metadata.hash, blob3.metadata.hash, "Different content should have different hash");
    println!("   ✓ Hash uniqueness verified: {} != {}", 
        &blob1.metadata.hash[..8], &blob3.metadata.hash[..8]);
...
async fn test_crdt_operations() -> Result<()> {
    ...
    assert!(pos2.lamport_timestamp > pos1.lamport_timestamp, "Later operation should have higher timestamp");
...
async fn test_r2_storage() -> Result<()> {
    ...
    match R2Config::from_env() {
        Ok(config) => {
            ...
            match R2Storage::new(config) {
                Ok(_storage) => {
                    ...
                    println!("   ✓ Test blob created: {} ({} bytes)", 
                        &blob.hash()[..8], blob.size());
        ...
        Err(e) => {
            ...
            println!("   ℹ Set R2_ACCOUNT_ID, R2_BUCKET_NAME, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY");
        }
    }
