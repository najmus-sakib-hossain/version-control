        ...
        let (action, details) = match &op.op_type {
            OperationType::Insert { position, content, length } => {
                ...
                (
                    ...
                    format!(
                        "{}:{} +{} chars '{}'",
                        position.line, position.column, length, preview.green()
                    ),
                )
            }
            ...
            OperationType::Replace { position, new_content, .. } => {
                ...
                (
                    ...
                    format!(
                        "{}:{} → '{}'",
                        position.line, position.column, preview.green()
                    ),
                )
            }
            _ => return,
...
pub async fn detect_lsp_support() -> Result<bool> {
    ...
    if let Ok(home) = std::env::var("HOME").or_else(|_| std::env::var("USERPROFILE")) {
        let vscode_extensions = PathBuf::from(home)
            .join(".vscode")
            .join("extensions");
