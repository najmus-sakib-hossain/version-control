...
async fn ws_cross_process_broadcast() {
    ...
    let (ws_a, _) = tokio_tungstenite::connect_async(url.clone()).await.expect("ws A");
    let (ws_b, _) = tokio_tungstenite::connect_async(url.clone()).await.expect("ws B");
    ...
    use futures::SinkExt;
    write_a.send(tokio_tungstenite::tungstenite::Message::Text(text.into())).await.unwrap();
    ...
    while let Some(msg) = read_b.next().await {
        ...
        if let Ok(tokio_tungstenite::tungstenite::Message::Text(t)) = msg {
            ...
            if let Ok(o) = serde_json::from_str::<Operation>(&s) {
                if o.id == op_id { got_from_b = true; break; }
            }
        }
    }
